...
const Layout = ({children}) => {
  ...
  const SidebarMenuDropdown = ({text, icon, eventKey, callback}) => {
    ...
    return (
      <div 
        className={`sidebar-menu d-flex justify-content-between align-items-center mt-1 p-2 rounded ${isCurrentEventKey && 'open border'}`}
  ...
  const SidebarHead = () => (
    <>
      ...
      <img 
        src={LOGO} 
        className="logo align-self-center mt-3"
        alt="itdc logo"
      />
  ...
  const SidebarNavWrapper = ({children}) => {
    return (
      <div className="sidebar-nav px-3 pt-3">
        {children}
      </div>
    )
  }
    ...
    <div className="layout-wrapper">
      <SidebarWrapper>
        ...
        <Accordion defaultActiveKey={lastMenuOpen}>
          {MENU.map((res, index) => res.hak && res.hak.some(hak => role.includes(hak)) && (
            <Fragment key={index}>
              {res.type === 'menu' ? (
                <SidebarMenu 
                  key={index}
